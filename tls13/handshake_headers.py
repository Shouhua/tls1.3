...
class EncryptedExtensionHandshakePayload(HandshakePayload):
    ...
    def deserialize(klass, data: bytes):
        ...
        (length,) = struct.unpack("!H", bytes_buffer.read(2))
        # server_name: 0, early_data: 42
        while bytes_buffer.tell() < length + 2:
            (t,) = struct.unpack("!H", bytes_buffer.read(2))
            (content_len,) = struct.unpack("!H", bytes_buffer.read(2))
            bytes_buffer.read(content_len)
            if t == 42:  # early data
                early_data = True
        return EncryptedExtensionHandshakePayload(data, early_data)
...
class CertificateHandshakePayload(HandshakePayload):
    ...
    def deserialize(klass, data: bytes):
        bytes_buffer = BytesIO(data)
        (_request_context,) = struct.unpack(
            "b", bytes_buffer.read(1)
        )  # 1字节的请求上下文，来源于CertificateRequest
        (_certificate_length,) = struct.unpack(
        ...
        (_certificate_extensions_follow,) = struct.unpack(
            ">h", bytes_buffer.read(2)
        )  # 这里不处理证书的扩展
...
class NewSessionTicketHandshakePayload(HandshakePayload):
    ...
    def obfuscated_ticket_age(self):
        ...
        return (
            ((utcnow() - self.received_time).seconds * 1000) + self.ticket_age_add
        ) % (1 << 32)
